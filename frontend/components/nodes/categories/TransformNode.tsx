// src/nodes/categories/TransformNode.tsx
import { Handle, Position } from '@xyflow/react';
import { BsThreeDots } from "react-icons/bs";
import { MdOutlineDownloadDone } from "react-icons/md";
import { FiClock, FiBox } from 'react-icons/fi';
...
function TransformNode({ data, selected, type }: { data: { label: string; description?: string, nodeData?: Record<string, any> }, selected: boolean, type: string }) {

  const iconMap: Record<string, React.ReactNode> = {
    delay: <FiClock />,
  };

  const categoryColorClass = 'ios-icon-bg-transform';
  ...
  return (
    <div className={`w-72 bg-[var(--node-bg-color)] border ${selected ? 'border-[var(--nodes-item-hover-border)] ring-1 ring-[var(--nodes-item-hover-border)]' : 'border-[var(--node-border-color)]'} rounded-xl node-shadow transition-all duration-200 hover:node-active-shadow select-none relative`}>
      ...
      {/* Transform nodes have input on left, output on right */}
      <Handle
        type="target"
