...
const WorkspaceDetailPage = () => {
  ...
  // Handle auto-seeding dummy workflows for this workspace if none exist
  useEffect(() => {
    const existing = workflows.filter(w => w.workspaceId === workspaceId);
    if (existing.length === 0) {
      // Seed 3 realistic workflows
      const initialDummyWorkflows: Omit<WorkflowType, 'workspaceId'>[] = [
        {
          id: `wf-dummy-1-${workspaceId}`,
          name: "OpenAI Customer Auto-Responder",
          description: "Analyze incoming customer support tickets using GPT-4, detect sentiment, and save a draft responder template in Gmail.",
          isEnabled: true,
          createdAt: new Date(Date.now() - 3600000 * 24 * 5).toISOString(),
          updatedAt: new Date(Date.now() - 3600000 * 2).toISOString()
        },
        {
          id: `wf-dummy-2-${workspaceId}`,
          name: "Webhook to HubSpot Lead Sync",
          description: "Listen for new sign-ups via webhook, enrich user profile using Clearbit, and sync details to HubSpot contacts.",
          isEnabled: true,
          createdAt: new Date(Date.now() - 3600000 * 24 * 12).toISOString(),
          updatedAt: new Date(Date.now() - 3600000 * 24 * 4).toISOString()
        },
        {
          id: `wf-dummy-3-${workspaceId}`,
          name: "Slack Build Notification Dispatcher",
          description: "Listen for build triggers from GitHub Action Webhooks and post summary cards to the #engineering-ops Slack channel.",
          isEnabled: false,
      ...
      initialDummyWorkflows.forEach(wf => {
        addWorkflow({
          ...wf,
          workspaceId
        });
  ...
  }, [workspaceId, workflows, addWorkflow]);
